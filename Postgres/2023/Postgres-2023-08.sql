...
create sequence refdata_codesetscrossmaps_codesetcrossmapid_seq;
...
create sequence refdata_industrystd_eventtypes_groupers_seq;
...
CREATE TABLE refdata_codesets_crossmaps (
                                         codesetcrossmapid integer DEFAULT nextval('refdata_codesetscrossmaps_codesetcrossmapid_seq'::regclass) NOT NULL,
...
(
    industrystdeventtypesgroupersid integer DEFAULT nextval('refdata_industrystd_eventtypes_groupers_seq'::regclass) NOT NULL,
...
/*ALTER TABLE dataexisting_profession
    ADD FOREIGN KEY (professiontypeid)
        REFERENCES refdata_professiontypes (professiontypeid);
*/
ALTER TABLE dataexisting_profession
    ADD FOREIGN KEY (statusid)
        REFERENCES refdata_status (statusid);
...
CREATE INDEX dataexisting_profession_idx ON dataexisting_profession USING btree (professionid, professionname, statusid, createddate, createduser, registeredapp);

CREATE UNIQUE INDEX dataexisting_profession_uindex ON dataexisting_profession USING btree (professionname, registeredapp);
